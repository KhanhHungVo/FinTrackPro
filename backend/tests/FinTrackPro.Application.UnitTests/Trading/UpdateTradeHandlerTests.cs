...
public class UpdateTradeHandlerTests
{
    ...
    private static Trade CreateTestTrade(Guid userId) =>
        Trade.Create(userId, "BTCUSDT", TradeDirection.Long, 30000m, 35000m, 0.1m, 5m, null);

    public UpdateTradeHandlerTests()
    {
        _handler = new UpdateTradeCommandHandler(_context, _tradeRepository, _currentUser, _userRepository);
    ...
    public async Task Handle_ValidCommand_UpdatesTradeAndReturnsDto()
    {
        ...
        var command = new UpdateTradeCommand(
            ...
            2000m, 2500m, 1m, 10m, "Updated note\nSecond line");
    ...
    public async Task Handle_UserNotFound_ThrowsNotFoundException()
    {
        ...
        var act = async () => await _handler.Handle(
            new UpdateTradeCommand(Guid.NewGuid(), "BTCUSDT", TradeDirection.Long, 30000m, 35000m, 0.1m, 5m, null),
    ...
    public async Task Handle_TradeOwnedByAnotherUser_ThrowsAuthorizationException()
    {
        ...
        var act = async () => await _handler.Handle(
            new UpdateTradeCommand(trade.Id, "BTCUSDT", TradeDirection.Long, 30000m, 35000m, 0.1m, 5m, null),
            CancellationToken.None);
    ...
    public async Task Handle_DomainRuleViolation_ThrowsDomainException()
    {
        ...
        var act = async () => await _handler.Handle(
            new UpdateTradeCommand(trade.Id, "BTCUSDT", TradeDirection.Long, 30000m, 0m, 0.1m, 5m, null),
            CancellationToken.None);
